...
final class UnitsyncWrapper {
    ...
    init(springDirectory: URL) throws {
        let handle = try DynamicLibraryHandle(libraryPath: Self.url(for: springDirectory).path).throwIfNil()

        self.handle = handle

        // resolve the c functions we are going to use

        GetNextError = try handle.unsafeResolve("GetNextError")

        // Spring Version

        GetSpringVersion = try handle.unsafeResolve("GetSpringVersion")
        GetSpringVersionPatchset = try handle.unsafeResolve("GetSpringVersionPatchset")
        IsSpringReleaseVersion = try handle.unsafeResolve("IsSpringReleaseVersion")

        // Initialisation/Uninit

        Init = try handle.unsafeResolve("Init")
        UnInit = try handle.unsafeResolve("UnInit")

        // Data directory

        GetWritableDataDirectory = try handle.unsafeResolve("GetWritableDataDirectory")
        GetDataDirectoryCount = try handle.unsafeResolve("GetDataDirectoryCount")
        GetDataDirectory = try handle.unsafeResolve("GetDataDirectory")

        // Units

        ProcessUnits = try handle.unsafeResolve("ProcessUnits")
        GetUnitCount = try handle.unsafeResolve("GetUnitCount")
        GetUnitName = try handle.unsafeResolve("GetUnitName")
        GetFullUnitName = try handle.unsafeResolve("GetFullUnitName")

        // Archives

        AddArchive = try handle.unsafeResolve("AddArchive")
        AddAllArchives = try handle.unsafeResolve("AddAllArchives")
        RemoveAllArchives = try handle.unsafeResolve("RemoveAllArchives")
        GetArchiveChecksum = try handle.unsafeResolve("GetArchiveChecksum")
        GetArchivePath = try handle.unsafeResolve("GetArchivePath")

        // Maps

        GetMapCount = try handle.unsafeResolve("GetMapCount")
        GetMapInfoCount = try handle.unsafeResolve("GetMapInfoCount")
        GetMapName = try handle.unsafeResolve("GetMapName")
        GetMapFileName = try handle.unsafeResolve("GetMapFileName")
        GetMapMinHeight = try handle.unsafeResolve("GetMapMinHeight")
        GetMapMaxHeight = try handle.unsafeResolve("GetMapMaxHeight")
        GetMapArchiveCount = try handle.unsafeResolve("GetMapArchiveCount")
        GetMapArchiveName = try handle.unsafeResolve("GetMapArchiveName")
        GetMapChecksum = try handle.unsafeResolve("GetMapChecksum")
        GetMapChecksumFromName = try handle.unsafeResolve("GetMapChecksumFromName")
        GetMinimap = try handle.unsafeResolve("GetMinimap")
        GetInfoMapSize = try handle.unsafeResolve("GetInfoMapSize")
        GetInfoMap = try handle.unsafeResolve("GetInfoMap")

        // Skirmish AIs

        GetSkirmishAICount = try handle.unsafeResolve("GetSkirmishAICount")
        GetSkirmishAIInfoCount = try handle.unsafeResolve("GetSkirmishAIInfoCount")
        GetInfoKey = try handle.unsafeResolve("GetInfoKey")
        GetInfoType = try handle.unsafeResolve("GetInfoType")
        GetInfoValueString = try handle.unsafeResolve("GetInfoValueString")
        GetInfoValueInteger = try handle.unsafeResolve("GetInfoValueInteger")
        GetInfoValueFloat = try handle.unsafeResolve("GetInfoValueFloat")
        GetInfoValueBool = try handle.unsafeResolve("GetInfoValueBool")
        GetInfoDescription = try handle.unsafeResolve("GetInfoDescription")
        GetSkirmishAIOptionCount = try handle.unsafeResolve("GetSkirmishAIOptionCount")

        // Games/Mods

        GetPrimaryModCount = try handle.unsafeResolve("GetPrimaryModCount")
        GetPrimaryModInfoCount = try handle.unsafeResolve("GetPrimaryModInfoCount")
        GetPrimaryModArchive = try handle.unsafeResolve("GetPrimaryModArchive")
        GetPrimaryModArchiveCount = try handle.unsafeResolve("GetPrimaryModArchiveCount")
        GetPrimaryModArchiveList = try handle.unsafeResolve("GetPrimaryModArchiveList")
        GetPrimaryModIndex = try handle.unsafeResolve("GetPrimaryModIndex")
        GetPrimaryModChecksum = try handle.unsafeResolve("GetPrimaryModChecksum")
        GetPrimaryModChecksumFromName = try handle.unsafeResolve("GetPrimaryModChecksumFromName")

        // Factions

        GetSideCount = try handle.unsafeResolve("GetSideCount")
        GetSideName = try handle.unsafeResolve("GetSideName")
        GetSideStartUnit = try handle.unsafeResolve("GetSideStartUnit")

        // Map/Mod Options

        GetMapOptionCount = try handle.unsafeResolve("GetMapOptionCount")
        GetModOptionCount = try handle.unsafeResolve("GetModOptionCount")
        GetCustomOptionCount = try handle.unsafeResolve("GetCustomOptionCount")
        GetOptionKey = try handle.unsafeResolve("GetOptionKey")
        GetOptionScope = try handle.unsafeResolve("GetOptionScope")
        GetOptionName = try handle.unsafeResolve("GetOptionName")
        GetOptionSection = try handle.unsafeResolve("GetOptionSection")
        GetOptionDesc = try handle.unsafeResolve("GetOptionDesc")
        GetOptionType = try handle.unsafeResolve("GetOptionType")
        GetOptionBoolDef = try handle.unsafeResolve("GetOptionBoolDef")
        GetOptionNumberDef = try handle.unsafeResolve("GetOptionNumberDef")
        GetOptionNumberMin = try handle.unsafeResolve("GetOptionNumberMin")
        GetOptionNumberMax = try handle.unsafeResolve("GetOptionNumberMax")
        GetOptionNumberStep = try handle.unsafeResolve("GetOptionNumberStep")
        GetOptionStringDef = try handle.unsafeResolve("GetOptionStringDef")
        GetOptionStringMaxLen = try handle.unsafeResolve("GetOptionStringMaxLen")
        GetOptionListCount = try handle.unsafeResolve("GetOptionListCount")
        GetOptionListDef = try handle.unsafeResolve("GetOptionListDef")
        GetOptionListItemKey = try handle.unsafeResolve("GetOptionListItemKey")
        GetOptionListItemName = try handle.unsafeResolve("GetOptionListItemName")
        GetOptionListItemDesc = try handle.unsafeResolve("GetOptionListItemDesc")

        // Mod Valid Maps

        GetModValidMapCount = try handle.unsafeResolve("GetModValidMapCount")
        GetModValidMap = try handle.unsafeResolve("GetModValidMap")

        // VFS

        OpenFileVFS = try handle.unsafeResolve("OpenFileVFS")
        CloseFileVFS = try handle.unsafeResolve("CloseFileVFS")
        ReadFileVFS = try handle.unsafeResolve("ReadFileVFS")
        FileSizeVFS = try handle.unsafeResolve("FileSizeVFS")
        InitFindVFS = try handle.unsafeResolve("InitFindVFS")
        InitDirListVFS = try handle.unsafeResolve("InitDirListVFS")
        InitSubDirsVFS = try handle.unsafeResolve("InitSubDirsVFS")
        FindFilesVFS = try handle.unsafeResolve("FindFilesVFS")
        OpenArchive = try handle.unsafeResolve("OpenArchive")
        CloseArchive = try handle.unsafeResolve("CloseArchive")
        FindFilesArchive = try handle.unsafeResolve("FindFilesArchive")
        OpenArchiveFile = try handle.unsafeResolve("OpenArchiveFile")
        ReadArchiveFile = try handle.unsafeResolve("ReadArchiveFile")
        CloseArchiveFile = try handle.unsafeResolve("CloseArchiveFile")
        SizeArchiveFile = try handle.unsafeResolve("SizeArchiveFile")

        SetSpringConfigFile = try handle.unsafeResolve("SetSpringConfigFile")
        GetSpringConfigFile = try handle.unsafeResolve("GetSpringConfigFile")
        GetSpringConfigString = try handle.unsafeResolve("GetSpringConfigString")
        GetSpringConfigInt = try handle.unsafeResolve("GetSpringConfigInt")
        GetSpringConfigFloat = try handle.unsafeResolve("GetSpringConfigFloat")
        SetSpringConfigString = try handle.unsafeResolve("SetSpringConfigString")
        SetSpringConfigInt = try handle.unsafeResolve("SetSpringConfigInt")
        SetSpringConfigFloat = try handle.unsafeResolve("SetSpringConfigFloat")
        DeleteSpringConfigKey = try handle.unsafeResolve("DeleteSpringConfigKey")

        lpClose = try handle.unsafeResolve("lpClose")
        lpOpenFile = try handle.unsafeResolve("lpOpenFile")
        lpOpenSource = try handle.unsafeResolve("lpOpenSource")
        lpExecute = try handle.unsafeResolve("lpExecute")
        lpErrorLog = try handle.unsafeResolve("lpErrorLog")
        lpAddTableInt = try handle.unsafeResolve("lpAddTableInt")
        lpAddTableStr = try handle.unsafeResolve("lpAddTableStr")
        lpEndTable = try handle.unsafeResolve("lpEndTable")
        lpAddIntKeyIntVal = try handle.unsafeResolve("lpAddIntKeyIntVal")
        lpAddStrKeyIntVal = try handle.unsafeResolve("lpAddStrKeyIntVal")
        lpAddIntKeyBoolVal = try handle.unsafeResolve("lpAddIntKeyBoolVal")
        lpAddStrKeyBoolVal = try handle.unsafeResolve("lpAddStrKeyBoolVal")
        lpAddIntKeyFloatVal = try handle.unsafeResolve("lpAddIntKeyFloatVal")
        lpAddStrKeyFloatVal = try handle.unsafeResolve("lpAddStrKeyFloatVal")
        lpAddIntKeyStrVal = try handle.unsafeResolve("lpAddIntKeyStrVal")
        lpAddStrKeyStrVal = try handle.unsafeResolve("lpAddStrKeyStrVal")
        lpRootTable = try handle.unsafeResolve("lpRootTable")
        lpRootTableExpr = try handle.unsafeResolve("lpRootTableExpr")
        lpSubTableInt = try handle.unsafeResolve("lpSubTableInt")
        lpSubTableStr = try handle.unsafeResolve("lpSubTableStr")
        lpSubTableExpr = try handle.unsafeResolve("lpSubTableExpr")
        lpPopTable = try handle.unsafeResolve("lpPopTable")
        lpGetKeyExistsInt = try handle.unsafeResolve("lpGetKeyExistsInt")
        lpGetKeyExistsStr = try handle.unsafeResolve("lpGetKeyExistsStr")
        lpGetIntKeyType = try handle.unsafeResolve("lpGetIntKeyType")
        lpGetStrKeyType = try handle.unsafeResolve("lpGetStrKeyType")
        lpGetIntKeyListCount = try handle.unsafeResolve("lpGetIntKeyListCount")
        lpGetIntKeyListEntry = try handle.unsafeResolve("lpGetIntKeyListEntry")
        lpGetStrKeyListCount = try handle.unsafeResolve("lpGetStrKeyListCount")
        lpGetStrKeyListEntry = try handle.unsafeResolve("lpGetStrKeyListEntry")
        lpGetIntKeyIntVal = try handle.unsafeResolve("lpGetIntKeyIntVal")
        lpGetStrKeyIntVal = try handle.unsafeResolve("lpGetStrKeyIntVal")
        lpGetIntKeyBoolVal = try handle.unsafeResolve("lpGetIntKeyBoolVal")
        lpGetStrKeyBoolVal = try handle.unsafeResolve("lpGetStrKeyBoolVal")
        lpGetIntKeyFloatVal = try handle.unsafeResolve("lpGetIntKeyFloatVal")
        lpGetStrKeyFloatVal = try handle.unsafeResolve("lpGetStrKeyFloatVal")
        lpGetIntKeyStrVal = try handle.unsafeResolve("lpGetIntKeyStrVal")
        lpGetStrKeyStrVal = try handle.unsafeResolve("lpGetStrKeyStrVal")
